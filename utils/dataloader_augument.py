...
class Dataset_Union_ALL(Dataset): 
    def __init__(self, paths, mode='train', data_type='Tr', image_size=128, 
        ...
        self.mode = mode
        self.pcc = pcc
        self.augmentations = augmentation_transforms
...
class Dataset_Union_ALL_Val(Dataset_Union_ALL):
    def __init__(self, paths, mode='train', data_type='Tr', image_size=128, 
                 transform=None, threshold=500, split_num=1, split_idx=0, 
                 pcc=False, augmentations=None):  # 添加augmentations参数
        # 首先处理子类特有的参数
        self.augmentations = augmentations
        # 然后调用父类构造函数，注意这里不再传递augmentations参数
        super().__init__(paths, mode, data_type, image_size, 
                         transform, threshold, split_num, split_idx, pcc)
