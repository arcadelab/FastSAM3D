...
class SwinTransformer(nn.Module):
    ...
    def __init__(
        ...
        self.upsample = nn.Sequential(
            # 增加转置卷积层进行上采样，将空间维度增加到标签的大小
            nn.ConvTranspose3d(6144, 3072, kernel_size=(2, 2, 2), stride=(2, 2, 2)),
            nn.ConvTranspose3d(3072, 1536, kernel_size=(2, 2, 2), stride=(2, 2, 2)),
            nn.ConvTranspose3d(1536, 384, kernel_size=(2, 2, 2), stride=(2, 2, 2)),
            # 最后使用1x1x1卷积将通道数匹配到标签的通道数
            nn.Conv3d(384, 384, kernel_size=(1, 1, 1))
        )
    ...
    def forward(self, x, normalize=True):
        # Transformer的前半部分保持不变
        x0 = self.patch_embed(x)
        x0 = self.pos_drop(x0)
        x0 = self.proj_out(x0, normalize) if normalize else x0
        if self.use_v2:
            x0 = self.layers1c[0](x0.contiguous())
        x1 = self.layers1[0](x0.contiguous())
        x1 = self.proj_out(x1, normalize) if normalize else x1
        if self.use_v2:
            x1 = self.layers2c[0](x1.contiguous())
        x2 = self.layers2[0](x1.contiguous())
        x2 = self.proj_out(x2, normalize) if normalize else x2
        if self.use_v2:
            x2 = self.layers3c[0](x2.contiguous())
        x3 = self.layers3[0](x2.contiguous())
        x3 = self.proj_out(x3, normalize) if normalize else x3
        x_up = self.upsample(x3)
        #up sampling to match label
        return x_up
