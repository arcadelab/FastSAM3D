...
from segment_anything.build_sam3D_moe import sam_model_registry3D
import argparse
...
parser.add_argument('--checkpoint', type=str, default='./work_dir/SAM/sam_vit_b.pth')  # turbo model
parser.add_argument('--device', type=str, default='cuda')
parser.add_argument('--work_dir', type=str, default='./moe_finetune')  # store weight location
...
class BaseTrainer:
    ...
    def init_checkpoint(self, ckp_path):
        ...
        if last_ckpt:
            if self.args.multi_gpu:
                self.model.module.load_state_dict(last_ckpt['model_state_dict'], strict=False) #, strict=False
            else:
                self.model.load_state_dict(last_ckpt['model_state_dict'], strict=False)  #, strict=False
            if not self.args.resume:
                self.start_epoch = 0 
            else:
                self.start_epoch = last_ckpt['epoch']
                self.optimizer.load_state_dict(last_ckpt['optimizer_state_dict'], strict=False)
                self.lr_scheduler.load_state_dict(last_ckpt['lr_scheduler_state_dict'], strict=False)
    ...
    def save_checkpoint(self, epoch, state_dict, describe="last"):
        torch.save({
            "epoch": epoch + 1,
            "model_state_dict": state_dict,      #状态字典，val的时候去load
