...
parser.add_argument('-sn', '--save_name', type=str, default='./results/sam_med3d.json')
...
if __name__ == "__main__":   
    ...
    with open(args.save_name, 'w') as f:
        ...
        for i in memory_befores:
            f.writelines(f'\'{str(i)},\n')
        f.writelines('memory_encoder')
        for i in memory_decoders:
            f.writelines(f'\'{str(i)},\n')
        f.writelines('memory_decoder')
        
        for i in FLOPSS:
            f.writelines('average flops')
            f.writelines(f'\'{str(np.average(i))},\n')
            for j in i:
                f.writelines(f'\'{str(j)},\n') 
        f.writelines('}')
