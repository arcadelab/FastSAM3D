...
import time
from segment_anything.modeling.imageencoder_moe import ImageEncoderViT3D
...
if __name__ == "__main__":   
    ...
    if(args.dim==3):
        ...
        # sam_model_tune.image_encoder = tiny_vit
        if checkpoint_path is not None:
            model_dict = torch.load(checkpoint_path, map_location=device)  #strict false to avoid last layer moe error
            state_dict = model_dict['model_state_dict']
            sam_model_tune.load_state_dict(state_dict)
    elif(args.dim==2):
        args.sam_checkpoint = args.checkpoint_path
        sam_model_tune = sam_model_registry[args.model_type](args).to(device)
    # Load checkpoint at this point
    #tiny_vit_checkpoint_path = 'work_dir1/union_train/sam_model_latest.pth'
