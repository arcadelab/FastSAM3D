...
class WindowAttention(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        # 应用 FlashAttention
        # 注意：这里需要根据您实际的 FlashAttention 函数签名进行调整
        softmax_scale = 1.0 / math.sqrt(c // self.num_heads)
        print("qkv shape:", qkv.shape)
    
        out = flash_attn_qkvpacked_func(qkv, dropout_p=0.0, softmax_scale=softmax_scale, causal=False)
        # 重整输出维度以匹配原始格式
