...
class Block3D(nn.Module):
    ...
    def __init__(
        ...
        num_experts: int = 4,  # 新增参数，指定专家的数量
        slots_per_expert: int = 1,  # 新增参数，指定每个专家的槽位数量
    ) -> None:
        """
        Args:
        ...
        # 新增SoftMoE层
        self.soft_moe = SoftMoE(
            in_features=int(dim),  # SoftMoE的输入特征维度
            out_features=dim,  # SoftMoE的输出特征维度
            num_experts=num_experts,  # 专家数量
            slots_per_expert=slots_per_expert,  # 每个专家的槽位数量
        )
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # 在调用soft_moe之前调整x的维度
        b, d, h, w, c = x.shape
        # print("Before reshape for SoftMoE, x shape:", x.shape)
        x_flattened = x.permute(0, 4, 1, 2, 3).reshape(b, c, -1).permute(0, 2, 1)  # 调整为(b, m, d)
        # 调用soft_moe
        # print("After reshape for SoftMoE, x_flattened shape:", x_flattened.shape)
        x_processed = self.soft_moe(x_flattened)
        # 将处理后的x_processed重塑回原始的5维形状
        x_processed = x_processed.permute(0, 2, 1).reshape(b, c, d, h, w).permute(0, 2, 3, 4, 1)  # 调整回(b, d, h, w, c)
        # print("x processed shape:", x_processed.shape)
        # 确保x_processed可以与shortcut相加，此时它们应该有相同的形状     
        x = shortcut + x_processed
...
def add_decomposed_rel_pos(
    ...
    # print("rel_pos_d shape:", rel_pos_d.shape)
    # print("rel_pos_h shape:", rel_pos_h.shape)
    # print("rel_pos_w shape:", rel_pos_w.shape)
    ...
    #print("Before einsum for rel_d, r_q shape:", r_q.shape, "Rd shape:", Rd.shape)
    rel_d = torch.einsum("bdhwc,dkc->bdhwk", r_q, Rd)
    #print("After einsum for rel_d, rel_d shape:", rel_d.shape)
    rel_h = torch.einsum("bdhwc,hkc->bdhwk", r_q, Rh)
    rel_w = torch.einsum("bdhwc,wkc->bdhwk", r_q, Rw)
    

    #print("Before adding relative positions, attn shape:", attn.shape)
    attn = (
        attn.view(B, q_d, q_h, q_w, k_d, k_h, k_w) + rel_d[:, :, :, :, None, None] + rel_h[:, :, :, None, :, None] + rel_w[:, :, :,None,None, :]
    ).view(B, q_d * q_h * q_w, k_d * k_h * k_w)
    # print("After adding relative positions, attn shape:", attn.shape)
    return attn
...
def main():
    # 创建 ImageEncoderViT3D 模型的实例
    # 这里需要用到实际的参数，例如 img_size, patch_size 等
    image_encoder_vit3d = ImageEncoderViT3D()
    print(f"Number of parameters in ImageEncoderViT3D: {count_parameters(image_encoder_vit3d)}")

    # 创建 TinyViT 模型的实例
    # 这里同样需要用到实际的参数，例如 img_size, in_chans, num_classes 等
