...
depths = [2, 2, 2, 2]  # 示例深度
channels = [64, 128, 256, 512]  # 示例通道数
num_heads = [1, 2, 4, 8]  # 示例头数
window_sizes = [7, 7, 7, 7]  # 示例窗口大小
quant_size = 4  # 示例量化大小
num_classes = 13  # 示例类别数

model = Swin3DUNet(depths, channels, num_heads, window_sizes, quant_size,
                   num_classes=num_classes)

# 创建一个示例三维输入张量
input_features = torch.randn(16, 6)  # 假设有16个非零位置，每个位置有6个通道
input_coords = torch.randint(0, 50, (16, 4))  # 假设坐标在0到50之间

sp_tensor = SparseTensor(input_features, input_coords)

output = model(sp_tensor, sp_tensor)
print(output.shape)  # 输出模型的输出维度
