...
class BaseTrainer:
    ...
    def initialmodel(self, path='weights/teacher_model_weights.pth', freezelayer = ['pos_embed', 'patch_embed.proj.weight', 'patch_embed.proj.bias', 'neck.0.weight', 'neck.1.weight', 'neck.1.bias', 'neck.2.weight', 'neck.3.weight', 'neck.3.bias']):
        # 加载权重
        p = torch.load(path)
    ...
    def train_epoch(self, epoch):
        ...
        for step, (image3D, label) in enumerate(tbar):
            ...
            with my_context():
                ...
                with amp.autocast():
                    output = self.model(image3D.to(self.args.device))
                    print("Original output shape:", output.shape)

                    # 调整维度以匹配标签的形状
                    output_shape = output.shape  # 模型实际输出的形状
                    print("output shape:", output.shape)
                    print("label shape", label[self.curlayer].shape)

                    loss = self.seg_loss(output, label[self.curlayer])
