...
class ImageEncoderViT3D(nn.Module):
    def __init__(
        ...
        config = LSTransformerEncoderLayer.get_config(
            ...
            max_seq_len=img_size // patch_size * img_size // patch_size * img_size // patch_size,  # 根据3D图像分割调整序列长度
            hidden_size=768,
        ...
        def forward(self, x: torch.Tensor) -> torch.Tensor:
            ...
            x = self.encoder(x)
            # x = [1,16,16,16,768]
            x = self.neck(x.permute(0, 2, 1).view(x.size(0), self.img_size, self.img_size, self.img_size, -1))
...
def main():
    ...
    # 生成模拟数据
    # 输入数据的形状为(batch_size, in_chans, img_size, img_size, img_size)
    dummy_input = torch.randn(batch_size, in_chans, img_size, img_size, img_size)
    ...
    # 如果你想查看模型的总参数数量，可以使用以下命令
    # 注意：确保所有的子模块和参数都正确定义，否则这个命令可能会报错或给出不正确的结果
    print(f"Total number of parameters in model: {count_parameters(model)}")
