...
def build_model(args):
    ...
    # 如果你需要在这里打印模型的summary，请确保已经将模型移动到了正确的设备上
...
class BaseTrainer:
    ...
    def train_epoch(self, epoch):
        ...
        if self.args.multi_gpu:
            sam_model = self.model.module
            current_device = torch.device(f'cuda:{self.args.rank}')
            print(f"当前进程排名: {self.args.rank}, 使用设备: {current_device}")
        else:
            sam_model = self.model
            current_device = self.args.device
            #self.args.rank = -1
        ...
        for step, (image, label) in enumerate(tbar):
            ...
            image3D = image3D.to(current_device)
            label = [l.to(current_device) for l in label]
                
            print(f"Current process rank: {self.args.rank}")
            print(f"当前进程排名: {self.args.rank}, 使用设备: {current_device}")
            with amp.autocast():
                output = sam_model(image3D)
                print("output shape",output.shape)
                print("label shape",label[-1].shape)
                loss = self.seg_loss(output, label[-1])
...
def main_worker(rank, args):
    print(f"启动进程 {rank} 在 GPU {args.gpu_ids[rank]} 上")
