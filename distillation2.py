...
class BaseTrainer:
    def __init__(self, model, dataloaders, args):
        ...
        self.curlayer = 1
        self.losses = []
        self.teacher_weights = self.load_teacher_weights('weights/teacher_model_weights.pth')
        self.set_loss_fn()
        self.set_optimizer()
        ...
        self.initialmodel('ckpt/sam_med3d_turbo.pth')
        #self.initialmodel()#
    
    def initialmodel(self, path='ckpt/sam_med3d_turbo.pth'):
        # 在加载权重之前记录一些关键参数的值
        before_weights = {name: param.clone() for name, param in self.model.named_parameters()}

        # 加载权重
        p = torch.load(path)['model_state_dict']
        for name, parameter in self.model.named_parameters():
            if 'patch_embed.proj' in name:
                parameter.data = p.get('image_encoder.' + name).clone()
                parameter.requires_grad = False
        sam_model_tune = sam_model_registry3D['vit_b_ori'](checkpoint=None).to(device)
        sam_model_tune.load_state_dict(p)
        self.model = sam_model_tune.image_encoder

    def load_teacher_weights(self, path):
        # 直接加载权重文件
        model_dict = torch.load(path, map_location=self.args.device)
        # 直接返回加载的权重
        return model_dict
    ...
    def compare_weights(self):
        with torch.no_grad():
            for name, param in self.model.named_parameters():
                teacher_param = self.teacher_weights.get(name)
                if teacher_param is not None and not torch.equal(param, teacher_param):
                    print(f"Weight changed: {name}")
                else:
                    print(f"Weight unchanged or not found in teacher model: {name}")
