...
parser.add_argument(
    ...
    "--test_data_path",
    type=str,
    default="/content/drive/MyDrive/lighting_sam_3d/data/process_train_data/brats2",
)
parser.add_argument(
    "-vp",
    "--vis_path",
    type=str,
    default="/content/drive/MyDrive/paper_visual_results/totalseg0441/sammed_new",
)
parser.add_argument(
    "-cp",
    "--checkpoint_path",
    type=str,
    default="/content/drive/MyDrive/lighting_sam_3d/ckpt/sam_vit_b_01ec64.pth",
)
parser.add_argument(
    "-sn",
    "--save_name",
    type=str,
    default="/content/drive/MyDrive/lighting_sam_3d/reb/originalsam_bratsnew.py",
)
...
def batch_forward(
    sam_model, image_embedding, gt3D, low_res_masks, points=None, device="cuda"
):
    # device = "cuda"
...
def interaction(sam_model, image_embedding, gt3D, num_clicks):
    ...
    low_res_masks = F.interpolate(
        prev_masks.float(), size=(img_size // 4, img_size // 4, img_size // 4)
    )
    ...
    for num_click in range(num_clicks):
        ...
        points_input, labels_input, click_points, click_labels = get_points(
            click_type, prev_masks, gt3D, click_points, click_labels, device
        )
    ...
    for slice_idx in tqdm(range(img3D.size(-1)), desc="transverse slices", leave=False):
        ...
        if k == 0:
            ...
            for iter in range(num_clicks):
                ...
                new_points_co, _ = random_point_sampling(
                    mask_to_sample.cpu(), get_point=1
                )
                new_points_la = (
                    torch.Tensor([1]).to(torch.int64)
                    if (
                        true_masks[new_points_co[0, 1].int(), new_points_co[0, 0].int()]
                    )
...
def finetune_model_predict3D(
    ...
    with torch.no_grad():
        image_embedding, times = sam_model_tune.image_encoder(
            img3D.to(device)
        )  # (1, 384, 16, 16, 16)
    ...
    for num_click in range(num_clicks):
        ...
        with torch.no_grad():
            ...
            FLOPS[num_click] += profile(
                sam_model_tune.prompt_encoder,
                (
                    [points_input, labels_input],
                    None,
                    low_res_masks.to(device),
                ),
            )[0]
            start_time = time.time()
            torch.cuda.reset_max_memory_allocated(device)
            ...
            FLOPS[num_click] += profile(
                ...
                (
                    image_embedding,
                    sam_model_tune.prompt_encoder.get_dense_pe(),
                    sparse_embeddings,
                    dense_embeddings,
                    False,
                ),
...
if __name__ == "__main__":
    ...
    if args.dim == 3:
        ...
        sam_model_tune = sam_model_registry[args.model_type](args.checkpoint_path).to(
            device
        )
    ...
    for batch_data in tqdm(test_dataloader):
        for i in range(0, 1):
            ...
            if (
                sz[2] < args.crop_size
                or sz[3] < args.crop_size
                or sz[4] < args.crop_size
            ):
            ...
            pred_path = os.path.join(
                vis_root, os.path.basename(img_name[0]).replace(".nii.gz", click_suffix)
            )
            ...
            if 1 == 0:
                ...
                for iter in range(args.num_clicks):
                    ...
                    iou_list.append(
                        round(
                            compute_iou(medsam_seg, gt3D[0][0].detach().cpu().numpy()),
                            4,
                        )
                    )
                    dice_list.append(
                ...
                pt_path = os.path.join(
                    vis_root,
                    os.path.basename(img_name[0]).replace(".nii.gz", "_pt.pkl"),
                )
