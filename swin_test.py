...
# from contextlib import nullcontext
# from utils.data_paths import img_datas
# #from segment_anything.modeling.swin_transformer import SwinTransformer #
...
#     if (args.multi_gpu):
#         model = torch.nn.DataParallel(SwinTransformer(
#             in_chans = 1,
#             embed_dim = 48,
...
#     model = model.to(args.device).to(dtype=torch.float16)
#     return model
# def get_dataloaders(args):
#     train_dataset = distillation_data(image_path='./data/Train/images', label_path='./data/Train/label')
...
# #         # 测试随机3D数据
# #         random_data = torch.rand(1, 1, 128, 128, 128).to(args.device).to(dtype=torch.float16) # 转换为 fp16
# #         with torch.no_grad():
# #             model.eval()
# #             output = model(random_data)
# #             print("Output shape from random data:", output.shape)
...
# #     # 测试随机3D数据
# #     random_data = torch.rand(1, 1, 128, 128, 128).to(args.device).to(dtype=torch.float16)
# #   # 转换为 fp16
# #     with torch.no_grad():
# #         model.eval()
# #         output = model(random_data)
...
def build_model(args):
    ...
    return model
def get_dataloaders(args):
    train_dataset = distillation_data(image_path='./data/train/image', label_path='./data/train/label')
